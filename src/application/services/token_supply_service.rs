...
use crate::domain::providers::TokenSupplyProvider;
use crate::domain::repositories::TokenSupplyRepository;
use crate::domain::services::TokenSupplyService;
    ...
    R: TokenSupplyRepository,
{
    ...
    pub fn new(provider: S, repository: Data<R>) -> Self {
        Self {
            provider,
            repository,
        }
...
mod tests {
    ...
    impl MockRepository {
        ...
        fn preset_supply(&self, total_supply: &str, circulating_supply: &str) {
            let mut stored = self.stored_supply.lock().unwrap();
            *stored = Some(TokenSupply::new(total_supply, circulating_supply));
        }
    }
    ...
    fn create_test_service(
        ...
        let service = DefaultTokenSupplyService::new(provider, repo_data.clone());
    ...
    async fn test_fetch_token_supply_should_delegate_to_provider() {
        ...
        let (service, _) = create_test_service(false, false, total, circulating);
    ...
    async fn test_fetch_token_supply_should_propagate_provider_errors() {
        // Arrange.
        let (service, _) = create_test_service(true, false, "1000.00", "500.00");
        let token = create_token();
        let sources = Vec::new();
    ...
    async fn test_update_token_supply_should_store_in_repository() {
        // Arrange.
        let (service, _) = create_test_service(false, false, "0.00", "0.00");
        let total = "2000.00";
    ...
    async fn test_update_token_supply_should_propagate_repository_errors() {
        // Arrange.
        let (service, _) = create_test_service(false, true, "1000.00", "500.00");
    ...
    #[tokio::test]
    async fn test_get_token_supply_should_retrieve_from_repository() {
        // Arrange.
        let (service, repo_data) = create_test_service(false, false, "0.00", "0.00");

        let total = "2000.00";
        let circulating = "1000.00";
        repo_data.preset_supply(total, circulating);

        // Act.
        let result = service.get_token_supply().await;

        // Assert.
        assert!(result.is_ok(), "Expected successful supply retrieval");
        let supply = result.unwrap();
        assert_supply_values(&supply, total, circulating);
    }
