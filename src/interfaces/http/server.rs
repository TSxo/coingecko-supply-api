...
use super::routes::{circulating_supply, health, total_supply};
...
impl HttpApplication {
    ...
    pub async fn build(
        config: ServerConfig,
        data: Data<InMemoryTokenSupplyRepository>,
    ) -> Result<Self, Error> {
        ...
        let server = HttpServer::new(move || {
            App::new()
                ...
                .service(
                    web::scope("/v1")
                        .route("/circulating", web::get().to(circulating_supply))
                        .route("/total", web::get().to(total_supply)),
                )
